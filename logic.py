...
from config import DATABASE, DB_FILE
import sqlite3
...
def save_file(user_id, file_type, filename, title=None):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO photoandvideo (user_id, type, filename, title, created_at) VALUES (?, ?, ?, ?, ?)",
        (user_id, file_type, filename, title, datetime.now().isoformat())
    )
    conn.commit()
    conn.close()
...
def get_user_files(user_id, file_type):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, filename, title FROM photoandvideo WHERE user_id = ? AND type = ? ORDER BY id DESC",
        (user_id, file_type)
    )
